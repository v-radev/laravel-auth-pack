...
@section('content')
    ...
    <div class="row">
        <div class="col-md-2">
            {!! Form::open(['route' => [$authDashboardRoutes . 'users.update', $user->id], 'method' => 'put']) !!}
            ...
            <div class="form-group">
                ...
                <select multiple="multiple" name="user_permissions[]" id="user_permissions"
                        style="width: 250px; height: 350px;">
                    @if(count($permissions))
                        @foreach($permissions as $p)
                            <option value="{{$p->permission->id}}"
                                    @if(in_array($p->permission->id, $userPermissions))selected="selected"@endif>{{$p->permission->name}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
...
@section('script')
    {!!Html::script('vendor/authcluster/js/ajax-utilities.js')!!}
    ...
    <script type="text/javascript">
        ...
        var a = new Ajax(),
                requestParams = {
                    type: 'POST',
                    url: '{{route($authDashboardRoutes . 'ajax')}}'
                };
