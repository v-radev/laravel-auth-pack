...
?>
@extends($authViews . 'layouts.dashboard')
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-2">
            <!-- per page select -->
            {!! Form::open(['route' => [$authDashboardRoutes . 'users.index'], 'method' => 'GET', 'autocomplete' => 'off']) !!}
        ...
        <table class="table table-bordered table-hover">
            ...
            @foreach($users as $index => $user)
                <tr class="item">
                    ...
                    <td><a href="{{route($authProfileRoutes . 'show', [$user->username])}}" target="_blank" class="username">{{$user->username}}</a></td>
                    <?php $role = $user->role(); ?>
                    ...
                    <td>
                        <a href="{{route($authDashboardRoutes . 'users.edit', [$user->id])}}" target="_blank">Edit</a>
                        <a href="{{route($authProfileRoutes . 'edit', [$user->username])}}" target="_blank">Profile</a>
                        @include($authViews . '_partials/form/delete', ['route' => [$authDashboardRoutes . 'users.destroy', $user->id], 'delete' => 'Delete'])
                    </td>
                </tr>
            @endforeach
            </tbody>
...
@section('script')
    {!! Html::script('vendor/authcluster/js/jquery/jquery-ui.min.js') !!}
    {!! Html::script('vendor/authcluster/js/jplist/jplist.core.min.js') !!}
    {!! Html::script('vendor/authcluster/js/jplist/jplist.sort-bundle.min.js') !!}
    {!! Html::script('vendor/authcluster/js/jplist/jplist.filter-dropdown-bundle.min.js') !!}
    {!! Html::script('vendor/authcluster/js/jplist/jplist.jquery-ui-bundle.min.js') !!}

    <script type="text/javascript">
        $('document').ready(function () {

            var ajaxUsersUrl = '<?= route($authDashboardRoutes . 'users.index'); ?>';
