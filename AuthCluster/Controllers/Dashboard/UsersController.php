...
class UsersController extends DefaultController {
        ...
        if ( is_array($data['user_permissions']) && !empty($data['user_permissions']) ) {
            ...
            foreach ( $data['user_permissions'] as $permission ) {
                ...
                if ( $user->attachPermission($permission->name) === FALSE ){
                    Flash::error('You are trying to assign a disabled permission to this role!'); goto redirect;
                }
            }
        }
    ...
    public function destroy( $id )
    {
        ...
        if ( $userRolePermissions && $userRolePermissions->delete() ) { $msg .= ', with user permissions'; }
        if ( $userRole && $userRole->delete() ) { $msg .= ', with user role'; }
